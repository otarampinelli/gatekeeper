...
import { gitLines, blobHash } from './git.mjs'
...
function runOne(analyzer, { cwd, base, head, deep, cacheDir, hasNodeModules, ciAvailable }) {
  ...
  const cmd = String(analyzer.run)
    ...
    .replaceAll('{{changed_files}}', subject.map((f) => `'${f}'`).join(' '))
...
export function runAnalyzers({ analyzers, cwd, base, head, deep, cacheDir, ciAvailable = false }) {
  ...
  return analyzers.map((a) => {
    try {
      ...
      // Fail soft, always. A broken analyzer degrades evidence; it never aborts a review.
...
// A red board must not cost one API call per failure.
const MAX_LOG_FETCHES = 8

function gh(args, { cwd, timeout = 60_000 }) {
...
export function fetchCiEvidence({ cwd, headSha, baseRef, defaultBranch }) {
  ...
  for (const r of results) {
    ...
    if (!jobId) continue
    const log = gh(['api', `repos/{owner}/{repo}/actions/jobs/${jobId}/logs`], { cwd, timeout: 120_000 })
