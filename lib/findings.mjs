...
function parseFinding(block, check) {
  ...
  // Strip fenced blocks before reading the prose. A diff body's own `-`/`+` lines do not
  // start with `- `, so they survive the bullet filter and end up pasted into the claim
  // that the verifier is asked to refute.
...
// Exact structural dedup only. Merging two findings that describe genuinely different
// problems is worse than showing both, so anything short of an exact file+line+title
// match is reported as a possible duplicate for the model to judge.
