// These prompts used to be filled in by hand in the skill: thirty-plus substitutions a
// run. Filling a template has one right answer, so it belongs here, not in the model's judgment.

import { join } from 'node:path'

// A grouped agent needs more reach than a solo one, but not N times more: the checks
// share a diff, a manifest, and usually the same handful of suspicious files.
...
// A verifier must never be the agent that argued for the finding: batching just amortizes
// each spawn's fixed cost, the one-per-finding ratio was incidental to that.
