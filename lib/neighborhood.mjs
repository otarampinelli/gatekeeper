...
// File list comes from the manifest, already filtered by diff_exclude: re-deriving it
// from git here would silently reintroduce generated files.
export function buildNeighborhood({ manifest, cwd }) {
  ...
  for (const file of targets) {
    // Exported declarations only: a local `const` has no cross-file reference graph by
    // definition, so mapping it would produce a fake "no references anywhere" signal.
    const symbols = [
      ...new Set(
